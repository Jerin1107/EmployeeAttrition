...
hr <- read.csv("/Users/jerineasothomas/Downloads/EDA_Project/train.csv")

hr$Attrition <- as.factor(hr$Attrition)

hr <- hr %>%
  # Binary categorical
  mutate(across(c(Department, EducationField, JobRole, MaritalStatus), ~ as.factor(.))) %>%
  # Nominal categorical
  mutate(across(c(EnvironmentSatisfaction, JobSatisfaction,
                  RelationshipSatisfaction, WorkLifeBalance, BusinessTravel,
                  Education, JobInvolvement, JobLevel, StockOptionLevel,
                  PerformanceRating), ~ as.ordered(.))) %>%
  # Ordinal categorical
...
hr_recipe2 <- 
  recipe(data = train,formula = Attrition ~ Age + TotalWorkingYears + MonthlyIncome +
...
# Let's see if a lasso model out preforms the regular logistic regression model
...
metric_df %>%
  ...
  labs(title = "Model evaluation",y = "Accuracy", x = NULL) +
  scale_y_continuous(limits = c(0.5,1),oob = rescale_none) +
  geom_text(aes(label = round(.estimate,3)),vjust = 2)


----------------------
# # Create a logistic regression model to predict Attrition
# model <- glm(Attrition ~ Age + Gender + MaritalStatus, data = attrition_data, family = binomial)
# 
# summary(model)
# 
# # Create a data frame for plotting the predicted probabilities
# new_data <- data.frame(Age = rep(seq(from = min(attrition_data$Age), to = max(attrition_data$Age), length.out = 100), times = 6),
#                        Gender = rep(c("Female", "Male"), each = 300),
#                        MaritalStatus = rep(c("Divorced", "Married", "Single"), each = 200))
# 
# # Add predicted probabilities to the data frame
# new_data$Attrition <- predict(model, newdata = new_data, type = "response")
# 
# # Plot the predicted probabilities using ggplot
# ggplot(new_data, aes(x = Age, y = Attrition, color = MaritalStatus)) +
#   geom_line(size = 1.5) +
#   facet_wrap(~ Gender, ncol = 2) +
#   labs(x = "Age", y = "Probability of Attrition", color = "Marital Status") +
#   theme_minimal()
...
unique_metric_df <- metric_df %>%
  filter(model == "RF") %>%
  distinct(.metric, .estimator, .keep_all = TRUE)

final_metric_df <- bind_rows(unique_metric_df, metric_df %>% filter(model != "RF"))
...
-----------------
yes_attrition <- hrnew %>% filter(Attrition == "1")
no_attrition <- hrnew %>% filter(Attrition == "0") %>% sample_n(nrow(yes_attrition))
balanced_employee <- rbind(no_attrition, yes_attrition)


hrnew
employee_filtered <- balanced_employee %>% mutate(Attrition = ifelse(Attrition == "0", 0, 1))
employee_filtered <- employee_filtered %>% filter(JobSatisfaction==3, Department=='Research & Development')

plt1<-ggplot(employee_filtered, aes(x = MonthlyIncome, y = Attrition)) +
  geom_point() +
  geom_jitter(height = 0.1, alpha=0.1) + 
  geom_smooth(method = "glm", se=FALSE, aes(y = Attrition), color = "red", method.args = list(family = "binomial"), alpha=0.1)+ ggtitle('Research & Department') +
  theme_bw()

employee_filtered <- balanced_employee %>% mutate(Attrition = ifelse(Attrition == "0", 0, 1))
employee_filtered <- employee_filtered %>% filter(JobSatisfaction==3, Department=='Sales')

plt2<-ggplot(employee_filtered, aes(x = MonthlyIncome, y = Attrition)) +
  geom_point() +
  geom_jitter(height = 0.1, alpha=0.1) + 
  geom_smooth(method = "glm", se=FALSE, aes(y = Attrition), color = "red", method.args = list(family = "binomial"), alpha=0.1)+ ggtitle('Sales') +
  theme_bw() + theme()

employee_filtered <- balanced_employee %>% mutate(Attrition = ifelse(Attrition == "0", 0, 1))
employee_filtered <- employee_filtered %>% filter(JobSatisfaction==3, Department=='Human Resources')

plt3<-ggplot(employee_filtered, aes(x = MonthlyIncome, y = Attrition)) +
  geom_point() +
  geom_jitter(height = 0.1, alpha=0.1) + 
  geom_smooth(method = "glm", se=FALSE, aes(y = Attrition), color = "red", method.args = list(family = "binomial"), alpha=0.1)+
  theme_bw()+ggtitle('Human Resources')
...
plt1<-ggplot(employee_filtered, aes(x = MonthlyIncome, y = Attrition)) +
  ...
  geom_smooth(method = "glm", se=FALSE, aes(y = Attrition), color = "red", method.args = list(family = "binomial"), alpha=0.1)+ ggtitle('Research & Department') +
  theme_bw()+
  theme(plot.height = unit(8, "cm"), plot.width = unit(8, "cm"))
...
grid.arrange(plt1, plt2, plt3, nrow = 3, heights = c(4, 4, 4))
